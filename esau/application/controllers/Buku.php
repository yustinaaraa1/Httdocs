...
class Buku extends CI_Controller {
    ...
    {     
        try{
            ...
            $crud->columns('gambar_produk','sku','judul_buku','pengarang','penerbit','tahun_terbit','jumlah_buku');
            $crud->order_by('id_buku', 'DESC');
            $crud->fields('id_buku','id_kategori','sku','judul_buku','pengarang','penerbit','tahun_terbit','jumlah_buku','gambar_produk');
            $crud->change_field_type('id_buku','invisible');
            $crud->set_relation('id_kategori','tb_kategori', 'nama_kategori');
            $crud->set_field_upload('gambar_produk','assets/img');
